...
clientRoute.route('/update-client/:id').put(async (req, res, next) => {
  try {
    const { firstname, lastname, phone, email, date, time, place } = req.body;
    const client = await ClientModel.findByIdAndUpdate(req.params.id, { firstname, lastname, phone, email, date, time, place }, { new: true });
    res.json(client);
  } catch (error) {
    return next(error);
  }
